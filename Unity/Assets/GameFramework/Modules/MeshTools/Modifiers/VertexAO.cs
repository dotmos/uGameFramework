...
//Slightly modified Vertex Ambient Occlusion Baker from http://adrianswall.com/?p=28

namespace MeshTools{
	public partial class Modifier
	{
		public static void VertexAO(Mesh mesh, int samples, float minRange, float maxRange, float intensity = 1, bool resetExistingAlpha = true, bool averageNormals = false)
		{
			//Generate collider from mesh so we can shoot rays to create the ambient occlusion
			//This will be delete once ao generation is complete
			GameObject go = new GameObject("VertexAOBakerTemp");
//			foreach(Mesh m in meshes)
//			{
//				go.AddComponent<MeshCollider>().sharedMesh = m;
//			}
			go.AddComponent<MeshCollider>().sharedMesh = mesh;
			go.layer = LayerMask.NameToLayer("MeshTools");

			//Layermask to shoot AO rays to
			int layerMask = LayerMask.GetMask("MeshTools");
...
//			int numVerts = 0;
//			foreach (Mesh m in meshes)
//				numVerts += m.vertices.Length;
//			numVerts = mesh.vertices.Length;
...
//				Mesh mesh = mf.sharedMesh;
				...
				for (i=0; i<l; i++) {
					...
					Vector3 n = go.transform.TransformPoint( verts [ i ] + nrm );
...
//						bool isHit = false;
						...
						//if ( Physics.Raycast( v-(offset), ray, out hit, maxRange, layerMask ) ) {
						// Linecast version
						if ( Physics.Linecast( v-(offset*0.1f), v + ray, out hit, layerMask ) ) {
...
//			} // meshes
			...
			//Delete temporary AO baker object
		...
		//fix seams in AO (vertex color.a) for a non-watertight mesh / mesh with seams
		public static void FixVertexAOSeams(Mesh mesh)
		{
			...
			for(int i=0; i<connections.Length; ++i)
			{
				...
				{
					//color.a += colors[connection.connections[sv]].a;
					color.a = Mathf.Min(color.a, colors[connection.connections[sv]].a);
				}
				//color.a /= connection.connections.Count;
				colors[i] = color;
			}
