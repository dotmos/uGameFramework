...
//Taken from http://www.binpress.com/tutorial/creating-an-octahedron-sphere/162

namespace MeshTools{
	public partial class Generator{

		private class OctahedronHelper{
			public static void Normalize (Vector3[] vertices, Vector3[] normals) {
			...
			public static void CreateTangents (Vector3[] vertices, Vector4[] tangents) {
				for (int i = 0; i < vertices.Length; i++) {
					Vector3 v = vertices[i];
					v.y = 0f;
					v = v.normalized;
		...
		//Create an octahedron mesh, complete with normals, uvs and tangents
		public static Mesh Octahedron(int subdivisions, float radius)
		{
			...
			OctahedronHelper.CreateOctahedron(vertices, triangles, resolution);
			
			Vector3[] normals = new Vector3[vertices.Length];
			OctahedronHelper.Normalize(vertices, normals);
			
			Vector2[] uv = new Vector2[vertices.Length];
			OctahedronHelper.CreateUV(vertices, uv);
			
			Vector4[] tangents = new Vector4[vertices.Length];
			OctahedronHelper.CreateTangents(vertices, tangents);
