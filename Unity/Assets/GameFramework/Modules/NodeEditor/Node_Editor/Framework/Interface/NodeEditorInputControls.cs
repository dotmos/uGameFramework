...
namespace NodeEditorFramework 
{
	...
	public static class NodeEditorInputControls
	{
		...
		{ // Show all nodes, and if a connection is drawn, only compatible nodes to auto-connect
			...
			{
				if (NodeCanvasManager.CheckCanvasCompability (compatibleNode.GetNodeID, inputInfo.editorState.canvas) && inputInfo.editorState.canvas.CanAddNode (compatibleNode.GetNodeID))
					canvasContextMenu.AddItem (new GUIContent ("Add " + NodeTypes.nodes[compatibleNode].adress), false, CreateNodeCallback, new NodeEditorInputInfo (compatibleNode.GetNodeID, state));
			}
		}
		...
		private static void DuplicateNode (NodeEditorInputInfo inputInfo) 
		{
			inputInfo.SetAsCurrentEnvironment ();
			NodeEditorState state = inputInfo.editorState;
			if (state.focusedNode != null && NodeEditor.curNodeCanvas.CanAddNode (state.focusedNode.GetNodeID)) 
			{ // Create new node of same type
				Node duplicatedNode = Node.Create (state.focusedNode.GetNodeID, NodeEditor.ScreenToCanvasSpace (inputInfo.inputPos), state.connectOutput);
